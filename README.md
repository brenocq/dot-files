...
Monitor-specific settings are in `monitors.conf`, which is generated by nwg-displays and kept local to each machine (not tracked in git). Workspace settings are in `workspaces.conf`.
