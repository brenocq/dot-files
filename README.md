...
### Hyprland
Edit `arch/.config/hypr/hyprland.lua` for:
- Keybindings
- Window rules
- Animations
- Display settings

Monitor-specific settings are in `monitors.conf`, which is generated by nwg-displays and kept local to each machine (not tracked in git); `hyprland.lua` parses it at startup and skips it when missing. Hyprland 0.55+ reads `hyprland.lua` (the hyprlang `.conf` format is deprecated).
